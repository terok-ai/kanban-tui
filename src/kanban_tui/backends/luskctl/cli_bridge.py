...
def task_followup(project_id: str, task_id: str, prompt: str) -> bool:
    """Send followup prompt to a stopped autopilot task. Returns True on success."""
    try:
        result = _run_luskctl(
            "task", "followup", project_id, task_id,
            "--prompt", prompt, "--no-follow",
            timeout=60,
        )
        return result.returncode == 0
    except (FileNotFoundError, subprocess.TimeoutExpired):
